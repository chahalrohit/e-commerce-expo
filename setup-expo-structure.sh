# run command to create folder stucture
# chmod +x setup-expo-structure.sh
# ./setup-expo-structure.sh
...
mkd "__tests__"
# For Expo managed workflow, no ios/ and android/ by default.
# If you are in a bare Expo project, these may already exist—this script won't touch them.
...
# ---------- Expo entry & config ----------
# Use src/App.tsx as entry via app.json -> expo.entryPoint
...
# Minimal app.json that sets entryPoint if missing
if [ ! -f "app.json" ]; then
cat > app.json <<'EOF'
{
  "expo": {
...
echo "✅ Expo structure created (non-destructive)."
